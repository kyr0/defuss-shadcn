...
(function () {
    ...
    darkMQ.addEventListener('change', function (e) {
        if (localStorage.getItem('defuss-shadcn-theme'))
            return; // user chose manually — respect it
    ...
    var NAV = [
        ...
        { heading: 'Feedback &amp; Status', items: [
                ...
                { label: 'Radial Progress', href: 'radial-progress.html' },
    ...
    var BUILT = new Set([
        ...
        'button.html', 'toggle.html', 'toggle-group.html', 'button-group.html', 'toolbar.html', 'swap.html',
        ...
        'spinner.html', 'skeleton.html', 'progress.html', 'radial-progress.html', 'alert.html', 'alert-dialog.html',
        'toast.html',
        ...
        connectedCallback() {
            ...
            this.querySelectorAll('details[data-nav-section]').forEach(function (d) {
                d.addEventListener('toggle', function () {
                    if (d.dataset.navSection === 'Overview') {
                        d.open = true;
                        return;
                    }
                    var set = navCollapsed();
                    if (d.open)
                        set = set.filter(function (h) { return h !== d.dataset.navSection; });
                    else if (set.indexOf(d.dataset.navSection) === -1)
                        set.push(d.dataset.navSection);
                    try {
    ...
    function buildToc() {
        ...
        candidates.forEach(function (el) {
            ...
            if (el.closest('.page-header details'))
                return;
        ...
        headings.forEach(function (item) {
            ...
            if (!item.el.querySelector('.heading-anchor') && !item.el.closest('.preview')) {
                item.el.insertAdjacentHTML('afterbegin', '<a class="heading-anchor" href="#' + id + '" aria-label="Link to section: ' + item.text.replace(/"/g, '"') + '">§</a>');
            }
    ...
    function buildPrevNext() {
        ...
        for (var i = 0; i < allPages.length; i++) {
            if (allPages[i].href === currentPage) {
                idx = i;
                break;
            }
        }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (layoutWrap) {
            layoutWrap.insertAdjacentHTML('beforeend', '<aside class="site-toc"><div class="site-toc-content"></div></aside>');
        }
        /* Inject footer */
        if (layoutWrap) {
